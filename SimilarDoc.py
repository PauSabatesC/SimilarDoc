...
class general():
    ...
    def distansia(self):
        ...
        text = Label(root, text="The distance between the documents is: %f"%self.distance, bg='#758AE3', font="-weight bold")
...
if __name__ == "__main__":
    ...
    root.wm_title("SimilarDoc")

    root.configure(background='#D38777')

    conf = Button(root, text='Instructions', command=instructions, bg="#B85B96", font="-weight bold")
    conf.place(relx=0.0, rely=0.89)

    text = Label(root, text="Enter the name of the first file: ", font="-weight bold")
    text.grid(row=0, column=0,ipadx=80,padx=10, pady=20)
    text.configure(background='#DBAEA4')

    x = Entry(root,font="-weight bold")
    x.grid(row=0, column=1, columnspan=2, ipadx=70,padx=10)

    Button(root, text='Add file 1', command=Fichero1, bg="#F3A428", font="-weight bold").grid(row=0, column=4, columnspan=1, padx=10)

    text = Label(root, text="Enter the name of the second file: ", font="-weight bold")
    text.grid(row=1, column=0, ipadx=71,padx=10)
    text.configure(background='#DBAEA4')

    y = Entry(root,font="-weight bold")
    y.grid(row=1, column=1, columnspan=2, ipadx=70)

    Button(root, text='Add file 2', command=Fichero2, bg="#F3A428", font="-weight bold").grid(row=1, column=4, columnspan=1)

    conf = Button(root, text='Confirm', command=fitx.distansia, bg="#D43E1D", font="-weight bold", width=20)
    conf.place(relx=0.4, rely=0.56)
    root.mainloop()